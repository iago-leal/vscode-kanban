...
> Gerado pelo Reversa em 2026-08-04. Este arquivo é o contrato do registro de bugs deste projeto.
...
> Nota de projeto: `vscode-kanban` é uma extensão do VS Code distribuída como pacote `.vsix`.
> Um defeito só está resolvido quando a build local empacotada (`npm run package` seguido da
> reinstalação da `.vsix`) exibe o comportamento corrigido, e não apenas quando a suíte passa:
> o Webview compilado é artefato separado do código-fonte.
...
- Schema completo: ver `bug-schema` na documentação do Reversa (`.claude/skills/reversa-debugger/references/`).
